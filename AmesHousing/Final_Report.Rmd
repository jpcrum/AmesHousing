---
title: Using Neighborhood Level Data to Predict the Residential Sale Price of Properties
  in Ames, Iowa
...
The dataset we chose for our project contains information from the Ames Assessor’s Office on residential property sales that occurred in Ames, Iowa from 2006 to 2010 [@JSE].[3] The dataset was published in the *Journal of Statistics Education* for students and researchers alike to have an opportunity to practice predictive modeling on messy, real world data. The dataset contains 2930 observations of 80 variables and the unit of observation is a single property sale in Ames, Iowa in a given year.
...
```{r linreg output, message = FALSE, echo=FALSE, results='asis'}
...
```{r paul neighbhd fig, fig.cap = "Figure 4: Boxplot of Sale Price by Neighborhood", fig.align = 'center', fig.width = 8, fig.height = 5, echo = FALSE}
...
```{r pfb.model1, echo = FALSE, results='asis'}
#model 1
...
Model           | Test RMSE
----------------|-------------------------
Model 1         | \$`r paste(round(saleprice_RMSE_Paul.1))`
Model 2         | \$`r paste(round(saleprice_RMSE_Paul.2))`

Table: Table 3: Test RMSE of Heuristic Approach OLS Models
...
Because PCR methods work best with numerical data, we decided to drop all factor variables from our training dataset, keeping only integer and numerical datatypes, before running a model. The dataset contains `r dim(df_train_num)[2]` variables after dropping factors. Then, we generated a kitchen sink PCR model using all the remaining variables in the training dataset to predict saleprice. 
...
```{r validation components, fig.cap = "Figure 5: Boxplot of Sale Price by Neighborhood", fig.align = 'center', fig.width = 8, fig.height = 5, echo = FALSE}
...
```{r modelvaldiation, echo=FALSE, results='asis'}
# predict RMSE pcr1
...
# Random Forest Regression, Elastic Net Regularization, and Other Complex Models
...
```{r plotrf1, fig.cap = "Figure 6: Training RMSE of Random Forest by Number of Predictors (mtry)", fig.align = 'center', echo = FALSE}
# plot the number of randomly selected predictors
plot(modelrf1, xlab = "Number of Randomly Selected Predictors")
```

```{r plotrf2, fig.cap = "Figure 7: Top 20 Independent Variables by Importance", fig.align = 'center', echo = FALSE}
...
```{r enet plot, fig.cap = "Figure 8: RMSE of Elastic Net Regularization for Varying Levels of the Mixing Parameter", fig.align = 'center', echo = FALSE}
# plot RMSE vs alpha
plot(enet1)
```
...
```{r ridge plot, fig.cap = "Figure 9: RMSE of Elastic Net Regularization for Varying Levels of the Mixing Parameter", fig.align = 'center', echo = FALSE}
# plot RMSE vs lambda
plot(ridge)
```
...
```{r knn plot, fig.cap = "Figure 10: RMSE for Varying Numbers of Maximum Neighbors", fig.align = 'center', echo = FALSE}
...
```{r glmplot, fig.cap = "Figure 11: RMSE of Gradient Boosted Model at Different Numbers of Boosting Iterations", fig.align = 'center', echo = FALSE}
# plot RMSE vs alpha
...
```{r rmse models, fig.cap = "Figure 12: RMSE of Bootstrap Samples of The Trained Models", fig.align = 'center', echo = FALSE}
bwplot(resamps, metric = "RMSE")
```


```{r r2models, fig.cap = "Figure 13: R-squared of Bootstrap Samples of The Trained Models", fig.align = 'center', echo = FALSE}
bwplot(resamps, metric = "Rsquared")
```

# Evaluation of Cross-Validated Models
...
----------------|-------------------------
pcr.1           | $`r round(saleprice_RMSE.1, 2)`
pcr.7           | $`r round(saleprice_RMSE.2, 2)`
pcr.24          | $`r round(saleprice_RMSE.3, 2)`
...
Table: Table 4: Test RMSE of Machine Learning Models

<div style="color:#999999;">** Note that pcr.1, pcr.7, and pcr.24 were cross-validated using the pcr function's built-in instead of the custom folds</div>

&nbsp;

From *Table 4*, we can see that the *rf1* and *rf2* are likely the strongest and most reliable prediction models, as they much lower RMSE values than the other models. However, we do not use *rf2* as it is merely the same model as *rf1*. Model *rf2* was the same as the first in that there were no zero variance predictors to remove and a difference grid search was run. It would be improper to cherry-pick it as our ideal model for this reason. 
...
*Table 5* demonstrates the ability of our model to predict the sale price, given specific variables from the test data set. *Table 5* shows the predicted values for median and standard deviation are close to the median of the actual dataset, and *Figure 13* graphs the predicted sales price against the actual sales prices for the test dataset. *Table 5* shows that our model is fairly strong at predicting, with the predicted median very close to the true median. However, the predicted min and predicted max are nearly \$20,000 and \$50,000 off, demonstrating our model gets weaker when predicted extreme values. This is further demonstrated by *Figure 13*, as we can see the predicted values vary from the true values at low and, especially, high sale prices.

```{r predict_rf1_rf2, echo=FALSE, results='asis'}
df_test <- read.csv("test_housing.csv", header = TRUE)
...
```{r predictionplot, fig.cap = "Figure 13: Predication vs Actual Sale Price (blue line is OLS fit. black line is 45˚ line)", fig.align = 'center', echo = FALSE}
# plot RMSE vs alpha
...
## Prediction for individual house

Using the following input paramters (from an observation of the test data), the modelrf1 predicts the sale price to be \$`r round( predict(modelrf1, test_x[22,]))`. The true price of this house is $`r round(test_y[22,])`.
...
If we change the garage size from a `r test_x[22,'garage.cars']` car garage to a 3 car garage, model rf1 predicts price to increase to \$`r round(predict(modelrf1, update_test_x_1))` (a `r round( (predict(modelrf1, update_test_x_1) - predict(modelrf1, test_x[22,]) )/predict(modelrf1, test_x[22,]),4)*100`% change in sale price).


# Limitations

Our model is primarily constrained by external validity. The training data is limited to Ames, Iowa from 2006 to 2010, and our measure of prediction ability (RMSE) was conducted using test data from Ames, Iowa. Applying this model to other areas, such as New York City, to commercial markets, or to time periods before 2006 or after 2010, would likely create inaccurate predictions. Additionally, the financial crisis of 2007-2008 may make these models specific to this time period and region. Future work may compare Ames, Iowa to other housing markets or develop a model based off housing data from a larger region or at a national scope. This difficulty in developing these datasets is consistency in variables across regions, especially for subject features, like overall house quality (`overall.qual`).


# Conclusion

We attempted several approaches for modeling. We found that OLS linear regression models failed to meet all the assumptions needed for proper implementation---gross violations of the classical assumptions result in biased coefficient estimates. Using more complicated predictive models, we sacrificed interpretability of variable coefficients for the sake of a smaller, average prediction error. The most robust model was the random forest model (modelrf1, RMSE: \$`r round(RMSE_rf1, 2)`).
