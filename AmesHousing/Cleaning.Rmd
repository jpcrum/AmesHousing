---
title: "Cleaning Housing Data From Ames, Iowa"
author: "Michael Arango, Yeshwant Chillakuru, Jack Crum, Paul Brewster"
date: "5/1/2017"
output:
  html_document:
    highlight: textmate
...
# commented out the version Paul needs to use to run and kept normal implementation
# df <- read.xls('./data/AmesHousing.xls', stringsAsFactors = FALSE, perl = "C:/Strawberry/perl/bin/perl.exe")
df <- read.xls('./original_data/AmesHousing.xls', stringsAsFactors = FALSE)
glimpse(df)
```
...
names(df)
# save original number of obs and vars
startObs <- dim(df)[1]
endObs <- dim(df)[2]
```
...
The `ms.subclass` variable specifies the type of dwelling involved in the sale. The variable is coded as an integer and has a key that can be found in the documentation. There are `r n_distinct(df$ms.subclass)` types of dwellings included. Numbers in the 20 to 85 range represent different traditional style homes and others represent duplexes or other split homes. 
...
df %>% 
  group_by(street) %>% 
  dplyr::summarise(median_sale_price = median(saleprice), 
            n = n())
```

## Alley

There are `r sum(is.na(df$alley))` missing values in the alley variable, but that's because the documentation states `NA` means `No Alley`.
...
Coding the NAs to No Alley, fixes all these missing values. 
...
We dropped the 3 observations that did not have public utilities. Then, we dropped the utility variable. 
...
We decided to drop the `ms.subclass` variable as it is a combination of `bldg.type`, `house.style`, and `year.built`. 
...
ggplot(tbl, aes(x = factor(overall.qual), y = median_saleprice)) + 
  ...
  ggsave(filename = "overallquality.png", width = 5, height = 5)
```
...
These are all new houses that were remodeled the year after they were built and sold. 
...
Because `r round(2587/(2587+23+9+7+4),4) * 100`% of `roof.matl` (roof material) is `CompShg` with the remaining spread across 7 categories, we transformed the data to Comp.Shg or not.
```{r change roof matl}
# Change roof.matl to CompShg and Other
df$roof.matl <- ifelse(df$roof.matl == 'CompShg', 'CompShg', 'Other')
```

Due to similar reasoning as `roof.matl`, `heating` is transformed to GasA, GasW, and other.

```{r combine lvls of heating}
...
From the bar plot of `bsmt.exposure` it seems as if there is an unnamed level of the variable. Looking at median sale price by the type of basement exposure, we can see this is true. These 3 observations are dropped.

```{r bsmt exposure changes}
...
We decided to change the names of the electrical categories to shorter abbreviations.
...
#New category names
table(df$electrical)
# Replace these with "SB" (Mode Imputation)
...
As advised in the documentation for the original project, we have dropped all houses with an above ground living area greater than 4000 square feet, dropping a total of 5 observatinons. Three of them are true outliers (Partial Sales that likely don't represent actual market values) and two of them are simply unusual sales (very large houses priced relatively appropriately). 

## First Floor Square Feet

Continuous Variable, ranging from 334 to 3820, median = 1082, mean = 1155

Ordered the data by above ground area, checked the head and tail to compare 1st floor square feet and above ground living area to make sure they align logically.
...
Continuous variable, ranging from 0 to 1862, median = 0, mean = 333.8, indicating that at least 50% of homes sold in Ames, Iowa do not have second floors. I made a new variable called has.2nd.floor, which is a dummy variable where 0 = "no second floor" and 1 = "has second floor". 42.67% of homes sold in Ames have 2nd floors.

Made a table containing neighborhoods and the ratio of houses sold that had a second floor in each neighborhood
...
Continuous variable, ranging from 0 to 1064, median = 0, mean = 4.685, no missing values98.63%(2885/2925) of homes have 0 square feet of low quality finish.OldTown (10), IDOTRR (7), and SWISU (6) have the most homes with low quality finish, though there are not that many (40) total. 

```{r}
summary(df$low.qual.fin.sf)
no.low.qual <- df %>% filter(low.qual.fin.sf == 0)
nrow(no.low.qual) / nrow(df)
wherelow <- df %>% group_by(neighborhood) %>% filter(low.qual.fin.sf > 0) %>% dplyr::summarise(n = n())
wherelow
```
...
We arranged by above ground living area from lowest to high and checked the head and tail to make sure everything made sense. The smallest homes have low room counts and few other features whereas the biggest homes have high room counts and many features.
...
Decided to combine all bathroom variables (above ground and basement full baths and half baths) into one total bathroom variable, discrete, if we want to simplify our analysis regarding bathrooms. Note: we need to be wary of including all of these variables into our model as total.bath is a linear combination of other variables. It might be worth running a very simple housing regression model with this total.bath variable. 

```{r}
# Calculate total bath variable
...
There is most likely a strong correlation between number of bedrooms and total number of rooms.

```{r}
table(df$bedroom.abvgr)
bedless <- df %>% filter(bedroom.abvgr == 0) 
bedless
#Checking how many rooms are in each sale with 0 bedrooms (ABOVE GROUND)
...
There are seven total variables related to the home's garage:
...
## Fill in missing garage year built

```{r}
index <- which(is.na(df$garage.yr.blt))
df[index, 'garage.yr.blt'] <- "0"
# plot to see if it makes a difference to keep or not
test <- df %>% filter(garage.yr.blt != "No garage")
qplot(as.integer(test$garage.yr.blt), test$saleprice)
# convert to number
df$garage.yr.blt <- as.integer(df$garage.yr.blt)
```

A garage year built of 0 means there is no garage.
...
ggplot(df1, aes(x = total.sf, y = saleprice, color = as.factor(total.flr))) + geom_point(alpha = 0.3)
```
...
Misc Feature and Misc Val are connected fields. It looks like misc.val should be non-zero when a non-NA value exists in Misc,feature and should be zero when misc.feature is NA.   
...
```{r checking misc.feature}
index <- which(is.na(df$misc.feature))
df[index, 'misc.feature'] <- 'No Feature'
index <- which(df$misc.feature != "No Feature")
df[index, 'misc.feature'] <- 'Feature'
plotCategorical('misc.feature', df)
```


```{r checking misc feature and misc value}
...
# drop misc. value
df <- df %>% dplyr::select(-misc.val)
```

## Sales Type

For the sale type variable, we decided to trim the leading and trailing space of the categories. Then, after looking at the bar plot and boxplot of sale price by sale type, we combined the levels to warranty deeds, new sales (first owner), and all other sale types.  
...
# check boxplot of sale.type vs house age
ggplot(df, aes(x = factor(sale.type), y = house.age)) + geom_boxplot(aes(fill = factor(sale.type))) + theme(axis.text.x = element_text(angle = 0, size = 10), legend.position = "none")
# recode sale-type to be 3 different levels
index <- str_which(df$sale.type, "WD")
table(df[index, 'sale.type'])
df[index, 'sale.type'] <- "Warranty Deed"
# recode all non-WD and non-new to other
index <- which(df$sale.type != "Warranty Deed" & df$sale.type != "New")
df[index, 'sale.type'] <- "Other"
# plot new boxplots
ggplot(df, aes(x = factor(sale.type), y = saleprice)) + geom_boxplot(aes(fill = factor(sale.type))) + theme(axis.text.x = element_text(angle = 0, size = 10), legend.position = "none") +  scale_y_continuous(labels = dollar)
```
...
# boxplot of sale price vs sale condition
ggplot(df, aes(x = factor(sale.condition), y = saleprice)) + 
  geom_boxplot(aes(fill = factor(sale.condition))) + 
  theme(axis.text.x = element_text(angle = 0, size = 10), legend.position = "none") + 
  scale_y_continuous(labels = dollar)
# Change partial sales to normal sales
index <- which(df$sale.condition == "Partial")
df[index, 'sale.condition'] <- 'Normal'
# we see a significant difference in median sale price
temp <- df %>% group_by(sale.condition) %>% summarise(med_sale = median(saleprice))
temp
# Normal vs not normal
index <- which(df$sale.condition != "Normal")
df[index, 'sale.condition'] <- 'Not Normal'
```

When modeling, we can filter as needed based on sale condition to just include normal sales. 
...
# remove outliers
outlierMinMax <- quantile(df$saleprice, c(.01, .99))
df <- df[ df$saleprice > outlierMinMax[1] & df$saleprice < outlierMinMax[2],  ]
# end number of obs
end <- dim(df)[1]
```

`r init - end` observations were dropped.
...
endObs <- dim(df)[1]
endVars <- dim(df)[2]
# check to see if fixed all missing data
any(is.na(df))
```

Our cleaned dataset has `r endObs` observations of `r endVars` variables. Overall, we dropped `r startObs-endObs` observations (`r round((startObs-endObs)/startObs * 100, 2)`%).
...
```{r write to csv}
# cleaned housing file
write_csv(df, "cleaned_housing.csv")
write_csv(train, "train_housing.csv")
write_csv(test, "test_housing.csv")
```
